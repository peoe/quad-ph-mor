...
FILEPATH = resource_filename('quad_ph_mor', 'experiments/data/msd')
INITIALS = [
    'jump',
    'sine',
]
ERROR = 'msd_{initial}_errors.csv'
ENERGY = 'msd_{initial}_energies.csv'
...
def main():
    ...
    msd = MSDExperiment(log_to_file=False)
    msd.setup(order=100)
    fom = msd.fom
    fom_h2 = fom.h2_norm()
    ...
    for initial in INITIALS:
        ...
        for index, name in enumerate([ name for name in os.listdir(target_dir) if os.path.isfile(os.path.join(FILEPATH, f'{initial}/' + name)) ]):
            ...
            msd.logger.info(f'Evaluating file {index}...')
            ...
            for res in results:
                ...
                order = res['order']
                if order not in msd.rom_matrices:
                    msd.reduce(order)
                rom = msd.roms(order)
